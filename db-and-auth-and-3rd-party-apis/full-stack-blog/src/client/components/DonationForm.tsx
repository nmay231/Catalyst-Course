...
const DonationForm: React.FC<IDonationForm> = ({stripe}) => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            console.log(err)
            pushAlert({content: 'It looks like we are having difficulties receiving donations right now. 😞', type: 'warning'}, 5000)
        }
    }

    const handleRadioChange: React.ChangeEventHandler = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
        <Form action={handleSubmit} submitText="Donate!" className="col-lg-6 col-md-8 mx-auto mt-5 border rounded-lg shadow">
            ...
            <div className="btn-group btn-group-toggle m-3" data-toggle="buttons">
                {amounts.map(amt => 
                    <label className={'btn btn-primary' + (!custom && amount === amt ? ' active' : '')} key={amt}>
                        <input type="radio" name="amount" id={amt.toString()} checked={!custom && amount === amt} onChange={handleRadioChange} /> ${amt}
                    </label>
                )}
                <label className={'btn btn-primary' + (custom ? ' active' : '')}>
                    <input type="radio" name="amount" id="custom" checked={custom} onChange={handleRadioChange} /> Custom
                </label>
            </div>
            {custom &&
                <FormField
